...
test:
	...
	docker run --rm --volumes-from jenkins go-todo-rest:test go test -v -coverprofile=coverage/c.out

coverage:
	docker run --rm --volumes-from jenkins go-todo-rest:test go tool cover -html=coverage/c.out -o coverage/coverage.html
